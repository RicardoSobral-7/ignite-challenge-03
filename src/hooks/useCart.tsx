...
export function CartProvider({ children }: CartProviderProps): JSX.Element {
  ...
  const removeProduct = async (productId: number) => {
    try {
      ...
      if(product) {
        const newListWithProductRemoved = cart.filter((product) => product.id !== productId);

        setCart(newListWithProductRemoved);
        localStorage.setItem('@RocketShoes:cart', JSON.stringify(newListWithProductRemoved));
      } else {
        throw new Error();
      }
